...
compute_minimum_cost <- function(initial_state, constant_cost) {
  ...
  if (constant_cost) {
    ...
    cost <- sapply(potential_positions, function(pos) sum(abs(initial_state-pos)))
  } else {
    # PART TWO: increasing cost for movement
    cost <- sapply(potential_positions, function(pos) {
      sum(sapply(abs(initial_state-pos), function(diff) sum(1:diff)))})
  }
  minimum_cost_position <- potential_positions[which(cost==min(cost))]
