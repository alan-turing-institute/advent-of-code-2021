use std::{ops::Sub, collections::HashMap, cmp};
use regex::Regex;
use std::fs;
...
impl Line {
    ...
    fn from_str(input: &str) -> Self {
        ...
        Self::new(Point(cap[1].parse().unwrap(), cap[2].parse().unwrap()), Point(cap[3].parse().unwrap(), cap[4].parse().unwrap()))
    ...
    fn points_on_line(&self, diagonal: bool) -> Option<Vec<Point>> {
        ...
        match self.end - self.start {
            Point(0, _) => {
                ...
                return Some((start..=end).map(|y| Point(self.start.0, y)).collect::<Vec<Point>>());
            },
            Point(_, 0) => {
                let start = cmp::min(self.start.0, self.end.0);
                let end = cmp::max(self.start.0, self.end.0);
                return Some((start..=end).map(|x| Point(x, self.start.1,)).collect::<Vec<Point>>());
            }, 
            _ => {
...
";
    ...
    #[test]
    fn test_line_parser() {
        assert_eq!(Line::from_str("0,9 -> 5,9"), Line::new(Point(0, 9), Point(5,9)));
    }

    #[test]
    fn test_part_1s() {
        let input: Vec<Line> = EXAMPLE_INPUT.lines().map(|line| Line::from_str(line)).collect();
