...
impl FromStr for Tree {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for i in 0..s.len() {
            ...
            let last_pair_idx = stack
                .iter()
                .filter(| &x| matches!(value[*x], Num::Pair(_)))
                .map(|x| *x)
                .rev()
                .nth(0);

            // let last_pair_idx = if stack.is_empty() {None} else {Some(stack.len() - 1)};
            let last_pair = if stack.is_empty() {
                None
            } else {
                Some(&mut value[last_pair_idx.unwrap()])
            };
